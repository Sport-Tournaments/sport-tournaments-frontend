          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Link href="/dashboard/dashboard/tournaments" className="text-sm text-primary hover:underline">
                  {t('common.viewAll')}
                </Link>
              </div>
            </CardHeader>
                  ...
                  {recentTournaments.map((tournament) => (
                    <li key={tournament.id}>
                      ...
                      >
                        ...
                        <Badge variant={(tournament.status as string) === 'IN_PROGRESS' || (tournament.status as string) === 'ONGOING' ? 'success' : 'default'}>
                          {tournament.status}
                        </Badge>
                      </Link>
                    </li>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Link href="/dashboard/dashboard/registrations" className="text-sm text-primary hover:underline">
                  {t('common.viewAll')}
                </Link>
              </div>
            </CardHeader>
                ...
                <ul className="space-y-3">
                  {recentRegistrations.map((registration) => (
                    <li key={registration.id}>
                      <Link
                        href={`/dashboard/dashboard/registrations/${registration.id}`}
                        className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                      >
                        <div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Link
                href="/dashboard/dashboard/tournaments/create"
                className="flex flex-col items-center gap-2 p-4 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-primary hover:bg-primary/5 transition-colors"
              >
                <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                </svg>
                <span className="text-sm font-medium">{t('dashboard.createTournament')}</span>
              </Link>
              <Link
                href="/dashboard/dashboard/clubs/create"
                className="flex flex-col items-center gap-2 p-4 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-primary hover:bg-primary/5 transition-colors"
              >
                <svg className="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
